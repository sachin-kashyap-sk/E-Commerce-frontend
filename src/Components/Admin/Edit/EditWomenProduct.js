...
import {
  ...
  selectedWomenProductModal,
} from "../../ReduxSection/Women/WomenSlice";
import { useDispatch, useSelector } from "react-redux";
import { TextField, Button } from "@mui/material";
import classes from "../../../Styles/Admin/Home/AddProduct.module.css";
// import { UpdateMenProduct } from "../../BackendApi/Men/MenProduct";
import { UpdateWomenProduct } from "../../BackendApi/Women/Women";
const EditWomenProduct = ({ productId, setProduct }) => {
  const dispatch = useDispatch();

  const product = useSelector((state) =>
    state.Women.value.find((item) => item._id === productId)
  );

  const open = useSelector(selectedWomenProductModal);

  const handleClose = () => {
    dispatch(showWomenProductModal(false));
    setProduct(() => "");
  };
