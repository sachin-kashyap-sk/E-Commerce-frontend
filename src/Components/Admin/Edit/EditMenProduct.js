...
import {
  editProduct,
  showMenProductModal,
  selectedMenProductModal,
} from "../../ReduxSection/Mens/MensSlice";
...
const EditMenProduct = ({ productId, setProduct }) => {
  const dispatch = useDispatch();

  const product = useSelector((state) =>
    state.Men.value.find((item) => item._id === productId)
  );

  const open = useSelector(selectedMenProductModal);

  const handleClose = () => {
    dispatch(showMenProductModal(false));
    setProduct(() => "");
  };
